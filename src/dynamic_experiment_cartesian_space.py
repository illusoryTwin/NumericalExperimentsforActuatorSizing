...
trajectory_points = [[0.6, 0.2, 0.2, 0.3, 0.5, 0.5],
                     [0.6, 0.2, -0.2, 0.3, 0.5, 0.5],
                     [0.6, 0.0, 0.0, 0.3, 0.5, 0.5],
                     [0.6, 0.4, 0.0, 0.3, 0.5, 0.5]]
...
def ik_residual(x, target_pos):
    print("x", x)
    # data.qpos[:] = x
    print(data.qpos.shape, np.squeeze(x).shape)
    x = np.squeeze(x)
    data.qpos = x
...
for i in range(len(trajectory_points) - 1):
    ...
    data.qpos[:] = np.deg2rad([5, 5, 5, 5, 5, 5])
    ...
    for t in segment_time_range:
        ...
        # ik_target = lambda x: ik_residual(np.squeeze(x), desired_pos)
